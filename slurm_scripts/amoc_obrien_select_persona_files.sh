#!/bin/bash
#SBATCH --job-name=obrien_persona
...
OBRIEN_JSON="${PROJECT_ROOT}/amoc/obrien_exp/obrien_combined.json"
BASE_OUTPUT_DIR="/export/home/acs/stud/a/ana_daria.zahaleanu/to_transfer/output/extracted_triplets/obrien_llama"
RUN_ID="obrien_${SLURM_ARRAY_JOB_ID}"
RUN_OUTPUT_DIR="${BASE_OUTPUT_DIR}/${RUN_ID}"
mkdir -p "${RUN_OUTPUT_DIR}"

# Get all CSV files sorted
mapfile -t ALL_CHUNKS < <(ls "${CHUNKS_DIR}"/*.csv | sort)

# Skip the first 8 files 
CHUNK_FILES=("${ALL_CHUNKS[@]:2:6}")
...
OUTPUT_CSV="${RUN_OUTPUT_DIR}/obrien_scores_chunk_${SLURM_ARRAY_TASK_ID}.csv"
...
apptainer exec --nv \
    ...
    bash -c "
        cd '$PROJECT_ROOT'
        python3 -m amoc.obrien_exp.obrien_exp_personas \
            --persona-csv '$INPUT_FILE' \
            --obrien-json '$OBRIEN_JSON' \
